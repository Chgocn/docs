> 本文由 [简悦 SimpRead](http://ksria.com/simpread/) 转码， 原文地址 [www.cnblogs.com](https://www.cnblogs.com/lixuejian/p/17086398.html)
...
IntentService 是系统提供的 Service 子类，用于在后台串行执行耗时任务，在处理完所有任务后会自动停止，不必来手动调用 `stopSelf()` 方法。而且由于 IntentService 是四大组件之一，拥有较高的优先级，不易被系统杀死，因此适合用于执行一些高优先级的异步任务
...
EventBus 的 Github 上有这么一句介绍：[EventBus](https://link.juejin.cn?target=https%3A%2F%2Fgreenrobot.org%2Feventbus%2F "https://greenrobot.org/eventbus/") is a publish/subscribe event bus for Android and Java. 这说明了 EventBus 是普遍适用于 Java 环境的，只是对 Android 系统做了特殊的平台支持而已。EventBus 的四种消息发送策略包含了 `ThreadMode.MAIN` 用于指定在主线程进行消息回调，其内部就是通过 Handler 来实现的
...
当 Looper 消费了 Message 后会调用 `recycleUnchecked()`方法将 Message 进行回收，在清除了各项资源后会缓存到 sPool 变量上，同时将之前缓存的 Message 置为下一个节点 next，通过这种链表结构来缓存最多 50 个 Message。这里使用到的是**享元设计模式**
...
作者：业志陈  
链接：[https://juejin.cn/post/6901682664617705485](https://juejin.cn/post/6901682664617705485)  
来源：稀土掘金  
著作权归作者所有。商业转载请联系作者获得授权，非商业转载请注明出处。

*   [一、动手实现 Handler](#一动手实现-handler)
*    [1、Message](#tid-j45xct)
*    [2、MessageQueue](#tid-nGYMJa)
*    [3、Handler](#tid-QH2j6f)
*    [4、Looper](#tid-DA7a2p)
*    [5、做一个总结](#tid-DhJXWr)
*   [二、Handler 源码](#二handler-源码)
*    [1、Handler 如何初始化](#tid-s7hdcN)
*    [2、Looper 如何初始化](#tid-X4K7Mi)
*    [3、Handler 发送消息](#tid-KAM4cf)
*    [4、MessageQueue](#tid-3jNeZK)
*    [5、消息屏障](#tid-dZJixH)
*    [6、退出 Looper 循环](#tid-TrArdb)
*    [7、IdleHandler](#tid-EAwdXM)
*    [8、做一个总结](#tid-5zATP7)
*   [三、Handler 在系统中的应用](#三handler-在系统中的应用)
*    [1、HandlerThread](#tid-FFezNM)
*    [2、IntentService](#tid-xdaaWN)
*   [四、Handler 在三方库中的应用](#四handler-在三方库中的应用)
*    [1、EventBus](#tid-jGKe3k)
*    [2、Retrofit](#tid-bK6hNK)
*   [五、提问环节](#五提问环节)
*    [1、Handler、Looper、MessageQueue、Thread 的对应关系](#tid-zSDjDy)
*    [2、Handler 的同步机制](#tid-R7RPey)
*    [3、Handler 发送同步消息](#tid-7Cj8yb)
*    [4、Handler 避免内存泄漏](#tid-SYAzEm)
*    [5、Message 如何复用](#tid-aeEaEc)
*    [6、Message 复用机制存在的问题](#tid-8YdnYi)
*    [7、Message 如何提高优先级](#tid-adWJJR)
*    [8、检测 Looper 分发 Message 的效率](#tid-K2FCC7)
*    [9、主线程 Looper 在哪里创建](#tid-t7XwtC)
*    [10、主线程 Looper 什么时候退出循环](#tid-KKRpMj)
*    [11、主线程 Looper.loop() 为什么不会导致 ANR](#tid-fNp6jC)
*    [12、子线程一定无法弹 Toast 吗](#tid-KHXYyY)
*    [13、子线程一定无法更新 UI？主线程就一定可以？](#tid-CXWydi)
*    [14、为什么 UI 体系要采用单线程模型](#tid-k6msZ5)
*    [15、如何跨线程下发任务](#tid-G4znWD)
*    [16、如何判断当前是不是主线程](#tid-SZmkKs)
*    [17、如何全局捕获主线程异常](#tid-ieKCzs)
...
*   **本文链接：** [https://www.cnblogs.com/lixuejian/p/17086398.html](https://www.cnblogs.com/lixuejian/p/17086398.html)
